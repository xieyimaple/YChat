...
import { ConversationType, ImageMessage, MessageContent, ObjectName, ReceiveMessage, TextMessage } from 'rongcloud-react-native-imlib';
import { YCChat } from '..';
...
export class YCRongCloudServe extends YCObject {
	...
	public async connect(token: string): Promise<string> {
		return new Promise((resolve, reject) => {
			const onSuccess = (userId: string) => {
				...
				this._ReceiveMessageListener = IMClient.addReceiveMessageListener(message => {
					console.log('接收消息');
					console.log(message);
	...
	private sendMessage(conversationType: ConversationType, targetId: string, content: MessageContent, pushContent: string, pushData: string): Promise<number> {
		return new Promise((resolve, reject) => {
	 ...
	 * @description 发送文字消息
	 */
	public async sendTextMessage(conversationType: ConversationType, targetId: string, message: string, pushContent = '', pushData = ''): Promise<number> {
		try {
			const content: TextMessage = {
				objectName: ObjectName.Text,
				userInfo: {
					userId: this.owner.currentUser.id,
					name: this.owner.currentUser.name,
					portraitUrl: this.owner.currentUser.photoUrl
				},
				content: message
			};
			const messageId = await this.sendMessage(conversationType, targetId, content, pushContent, pushData);
	...
	public async sendImageMessage(
		...
		try {
			...
			const messageId = await this.sendMediaMessage(conversationType, targetId, content, pushContent, pushData, callback);
